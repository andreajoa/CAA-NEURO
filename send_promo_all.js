...
async function run() {
  ...
  console.log(`✅ ${users.length} usuários encontrados. Iniciando disparos...`);

  for (const user of users) {
    const email = user.email_addresses[0]?.email_address;
    const firstName = user.first_name || 'amigo(a)';
    if (!email) continue;
    ...
    try {
      await resend.emails.send({
        ...
        html: html.replace(/amigo\(a\)/g, firstName)
      });
      console.log(`📧 Enviado para: ${email} (${firstName})`);
    } catch (err) {
