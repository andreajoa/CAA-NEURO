  ...
  return (
    <div style={{minHeight:"100vh",background:"#f0fdf4",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui",padding:"24px"}}>
      <div style={{background:"white",borderRadius:"24px",padding:"48px",maxWidth:"440px",width:"100%",textAlign:"center",boxShadow:"0 8px 40px rgba(0,136,95,0.15)"}}>
        {status==="loading" && <><div style={{fontSize:"48px",marginBottom:"16px"}}>⏳</div><h1 style={{fontSize:"20px",fontWeight:"700",color:"#1B2D5B"}}>Verificando convite...</h1></>}
        {status==="sucesso" && (<>
          <div style={{fontSize:"64px",marginBottom:"16px"}}>🎉</div>
          <h1 style={{fontSize:"22px",fontWeight:"900",color:"#1B2D5B",marginBottom:"8px"}}>Convite aceito!</h1>
          <p style={{color:"#6b7280",marginBottom:"24px"}}>Você agora faz parte de <strong>{mensagem}</strong> no CAA Neuro.</p>
          <button onClick={()=>router.push("/app")} style={{background:"#C76B4A",color:"white",border:"none",padding:"14px 28px",borderRadius:"12px",fontWeight:"700",cursor:"pointer",width:"100%",fontSize:"15px"}}>
            Ir para a prancha →
          </button>
        </>)}
        {status==="erro" && (<>
          <div style={{fontSize:"48px",marginBottom:"16px"}}>❌</div>
          <h1 style={{fontSize:"20px",fontWeight:"700",color:"#1B2D5B",marginBottom:"8px"}}>Convite inválido</h1>
