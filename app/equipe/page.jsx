...
export default function Equipe() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f9fafb",fontFamily:"system-ui"}}>
      <div style={{background:"#1B2D5B",padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      ...
      <div style={{maxWidth:"700px",margin:"0 auto",padding:"32px 24px"}}>
        <h1 style={{fontSize:"26px",fontWeight:"800",color:"#1B2D5B",marginBottom:"8px"}}>👥 Minha Equipe</h1>
        ...
        {!loading && !org && (
          <div style={{background:"white",borderRadius:"16px",border:"1px solid #e5e7eb",padding:"28px"}}>
            <h2 style={{fontSize:"18px",fontWeight:"700",color:"#1B2D5B",marginBottom:"8px"}}>Criar organização</h2>
            ...
            <div style={{display:"flex",gap:"10px",flexWrap:"wrap"}}>
              ...
              <button onClick={criarOrg} disabled={creating} style={{...btn,background:"#C76B4A",color:"white"}}>
                {creating ? "Criando..." : "Criar"}
              </button>
            </div>
            {msg && <p style={{marginTop:"12px",color:msg.type==="ok"?"#C76B4A":"#ef4444",fontSize:"14px"}}>{msg.text}</p>}
          </div>
        )}
        ...
        {!loading && org && (
          <>
            <div style={{background:"white",borderRadius:"16px",border:"1px solid #e5e7eb",padding:"24px",marginBottom:"24px"}}>
              ...
              <div style={{fontSize:"20px",fontWeight:"800",color:"#1B2D5B"}}>{org.nome}</div>
            </div>

            <div style={{background:"white",borderRadius:"16px",border:"1px solid #e5e7eb",padding:"24px",marginBottom:"24px"}}>
              <h2 style={{fontSize:"16px",fontWeight:"700",color:"#1B2D5B",marginBottom:"16px"}}>✉️ Convidar profissional</h2>
              <div style={{display:"flex",gap:"10px",flexWrap:"wrap"}}>
                <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="email@clinica.com"
                  style={{flex:1,padding:"10px 14px",borderRadius:"8px",border:"1px solid #e5e7eb",fontSize:"14px",minWidth:"200px"}} />
                <button onClick={convidar} disabled={sending} style={{...btn,background:"#C76B4A",color:"white"}}>
            ...
            <div style={{background:"white",borderRadius:"16px",border:"1px solid #e5e7eb",padding:"24px"}}>
              <h2 style={{fontSize:"16px",fontWeight:"700",color:"#1B2D5B",marginBottom:"16px"}}>👤 Membros ({members.length})</h2>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
                {members.map((m,i) => (
                  <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",background:"#f9fafb",borderRadius:"10px"}}>
                    <div>
                      <div style={{fontWeight:"700",color:"#1B2D5B",fontSize:"14px"}}>{m.user_id}</div>
