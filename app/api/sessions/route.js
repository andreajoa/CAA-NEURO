...
export async function POST(request) {
  ...
  if (!userId) return Response.json({ error: "Unauthorized" }, { status: 401 });
  try {
    const { patient_id, cards_usados, evolucao_observada, notas, objetivos_sessao, duracao_minutos } = await request.json();
    ...
    const r = await db.prepare(
      `INSERT INTO sessions (user_id,patient_id,cards_usados,evolucao_observada,notas,objetivos_sessao,duracao_minutos,created_at)
       VALUES (?,?,?,?,?,?,?,datetime('now'))`
    ).bind(userId,patient_id,JSON.stringify(cards_usados||[]),evolucao_observada||null,notas||null,objetivos_sessao||null,duracao_minutos||null).run();
