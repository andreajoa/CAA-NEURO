...
export async function POST(request) {
  ...
  try {
    ...
    if (!Array.isArray(cards) || !cards.length) return Response.json({ error: "Cards obrigatórios" }, { status: 400 });
    if (cards.length > 200) return Response.json({ error: "Máximo de 200 cards por prancha" }, { status: 400 });
    ...
    return Response.json({ token, url: `${appUrl.replace(/\/$/, "")}/prancha/${token}` });
...
export async function GET(request) {
  ...
  try {
    ...
    const board = rows[0];
    const cards = JSON.parse(board.cards || "[]");
    return Response.json({
      board: { token: board.token, profile: board.profile, level: board.level, title: board.title, cards },
