...
export default function PranchaPublica() {
  ...
  if (!board) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"system-ui", background:"#f9fafb" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <h2 style={{ color:"#1B2D5B" }}>Prancha não encontrada</h2>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f9fafb", fontFamily:"system-ui" }}>
      <div style={{ background:"#1B2D5B", padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"8px" }}>
        <div style={{ color:"#E8B4A8", fontWeight:"800", fontSize:"18px" }}>CAA Neuro</div>
      ...
      <div style={{ padding:"16px 24px", background:"white", borderBottom:"1px solid #e5e7eb" }}>
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap", minHeight:"40px", alignItems:"center" }}>
          ...
          {phrase.map((w, i) => (
            <span key={i} style={{ background:"#C76B4A", color:"white", padding:"6px 14px", borderRadius:"999px", fontSize:"15px", fontWeight:"600" }}>{w}</span>
          ))}
        </div>
        <div style={{ display:"flex", gap:"8px", marginTop:"10px" }}>
          <button onClick={() => speak(phrase.join(" "))}
            style={{ background:"#C76B4A", color:"white", border:"none", padding:"9px 20px", borderRadius:"8px", fontSize:"13px", fontWeight:"700", cursor:"pointer" }}>
            🔊 Falar frase
          </button>
      ...
      <div style={{ padding:"24px", display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))", gap:"16px" }}>
        {board.cards.map(card => (
          <button key={card.id} onClick={() => selectCard(card)}
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor="#C76B4A"; e.currentTarget.style.transform="scale(1.04)"; }}
            ...
            <span style={{ fontSize:"14px", fontWeight:"700", color:"#1B2D5B", textAlign:"center", lineHeight:"1.3" }}>{card.label}</span>
          </button>
        ))}
      </div>
