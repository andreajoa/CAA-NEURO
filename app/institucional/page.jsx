...
const NAV = () => (
  <nav style={{background:"#1B2D5B",padding:"10px 20px",display:"flex",gap:"10px",alignItems:"center",flexWrap:"wrap",borderBottom:"2px solid #C76B4A"}}>
    ...
    <a href="/institucional" style={{color:"white",textDecoration:"none",background:"rgba(78,201,160,0.3)",border:"1px solid #E8B4A8",padding:"7px 14px",borderRadius:"8px",fontSize:"13px",fontWeight:"600"}}>🏛️ Institucional</a>
  </nav>
);
...
const S = {
  ...
  title: {fontSize:"28px",fontWeight:"900",color:"#1B2D5B",margin:"0 0 4px"},
    ...
    <div style={S.page}>
      ...
      <div style={S.container}>
        ...
        <div style={S.card}>
          ...
          <button onClick={createOrg} disabled={loading||!form.nome} style={{...S.btn,background:"#C76B4A",color:"white",marginTop:"20px",opacity:loading||!form.nome?0.5:1}}>
  ...
  if (view === "detalhes" && selectedOrg) return (
    <div style={S.page}>
      ...
      <div style={S.container}>
        ...
        <div style={S.card}>
          <h2 style={{fontSize:"16px",fontWeight:"800",color:"#1B2D5B",marginBottom:"16px"}}>➕ Convidar profissional</h2>
          <div style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:"10px",alignItems:"end"}}>
            ...
            <button onClick={sendInvite} disabled={inviteLoading||!inviteEmail}
              style={{...S.btn,background:"#C76B4A",color:"white",opacity:inviteLoading||!inviteEmail?0.5:1,marginTop:"auto"}}>
        ...
        {/* Lista de membros */}
        <div style={S.card}>
          <h2 style={{fontSize:"16px",fontWeight:"800",color:"#1B2D5B",marginBottom:"16px"}}>
          ...
          {orgDetails?.members?.map(m=>(
            <div key={m.id} style={{display:"flex",alignItems:"center",gap:"12px",padding:"10px 0",borderBottom:"1px solid #f3f4f6"}}>
              <div style={{width:"36px",height:"36px",borderRadius:"50%",background:"#C76B4A",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:"700",fontSize:"14px",flexShrink:0}}>
    ...
    <div style={S.page}>
      ...
      <div style={S.container}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"28px",flexWrap:"wrap",gap:"12px"}}>
          ...
          <button onClick={()=>setView("nova")} style={{...S.btn,background:"#C76B4A",color:"white",fontSize:"15px",padding:"12px 24px"}}>
            + Nova organização
          </button>
        </div>
        ...
        {orgs.length===0 ? (
          <div style={{...S.card,textAlign:"center",padding:"60px 24px"}}>
            ...
            <button onClick={()=>setView("nova")} style={{...S.btn,background:"#C76B4A",color:"white",fontSize:"15px",padding:"12px 28px"}}>Criar primeira organização</button>
          </div>
        ) : orgs.map(org=>(
          <div key={org.id} style={{...S.card,display:"flex",alignItems:"center",gap:"16px",cursor:"pointer"}}
            ...
            <div style={{width:"48px",height:"48px",borderRadius:"12px",background:"#1B2D5B",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px",flexShrink:0}}>
              {org.tipo==="prefeitura"?"🏛️":org.tipo==="clinica"?"🏥":org.tipo==="escola"?"🏫":org.tipo==="hospital"?"🏨":"🤝"}
            </div>
            <div style={{flex:1}}>
              <div style={{fontWeight:"800",fontSize:"16px",color:"#1B2D5B"}}>{org.nome}</div>
