...
const STATIC = ["/app", "/manifest.json", "/icon-192-v2.png", "/icon-512-v2.png"];
    ...
    caches.open(CACHE).then(async cache => {
      ...
      await Promise.allSettled(
        CARDS_IMAGES.map(url =>
          fetch(url).then(res => {
            if (res.ok) cache.put(url, res); // ← sem clone, usa direto
          }).catch(() => {})
        )
      );
    ...
    e.respondWith(
      caches.match(e.request).then(cached => {
        ...
        return fetch(e.request).then(res => {
          if (res.ok) {
            const clone = res.clone(); // clone ANTES de consumir
    ...
    fetch(e.request)
      .then(res => {
        if (res.ok && res.type === "basic") {
          const clone = res.clone(); // clone ANTES de retornar
          caches.open(CACHE).then(c => c.put(e.request, clone));
        }
        return res;
      })
